...
const RaceGame = () => {
  ...
  const userShareData = useSelector((state) => state.addUserShareData);
  ...
  const stringHandler = (userInput) => {
    ...
    if (isMatch1) {
      ...
      setWrongWords((oldVal) => {
        originalStringArray[0] === oldVal[oldVal.length - 1]
          ? oldVal
          : oldVal.push(originalStringArray[0]);
        return oldVal;
      });
  ...
  }, [currentSocketSharedData, isGameBeingPlayed, isCounting]);
  ...
  useEffect(() => {
    ...
    return () => {
      socket.of("room_created");
      socket.of("match_found");
      socket.of("time_up");
      socket.disconnect();
    };
  }, [startCounting]);
  ...
  return (
    <>
      <div className="relative bg-gray-100 p-8 flex flex-col  2xl:w-[1300px] md:w-[500px] lg:w-[1000px] justify-center">
        <div
          className="absolute top-8 left-[45%] text-3xl font-semibold"
          id="title"
        >
          Typing Game
        </div>
     ...
     text-4xl font-bold`}
        ...
        {gameEnd && (
          <div className=" shadow-md my-5">
            {" "}
            <div className="text-center text-3xl ">Mistakes </div>
            <div className="mt-4 flex gap-6 justify-center flex-wrap  ">
              {wrongWords.map((wrongWord, index) => {
                return (
                  <div
                    key={index}
                    className="flex-[30%] text-[18px] px-1 font-semibold bg-blue-100 text-red-700"
                  >
                    {wrongWord}
                  </div>
                );
